...
- Секция с переменными (начинается с заголовка `section .data` ). Перменные могут копировать значение других, либо же содержать в себе адрес другой переменной (объявляется при помощи спец символа `&`). 
- Код программы (начинаается с заголовка `section .text` ). Набор команд, которые могут принимать на вход аргумент. Существует три типа аргументов:
